...
public interface EarmarkRepository extends CrudRepository<Earmark, Long>, JpaSpecificationExecutor<Earmark> {
	...
	List<Earmark> findAllocationEmployeeId(Long employeeId);

	@Query("select e from Earmark e where e.earmarkId in(:earmarkIds) and e.isActive = 'true'")
	List<Earmark> findByIds(List<Long> earmarkIds);

	List<Earmark> findByEarmarkIdIn(List<Long> earmarkIds);

	List<Earmark> findByAllocationEmployee(Employee emp);
	...
	void updateEarmark(Long earmarkId, Integer allocationPercentage, boolean isBillable,
					   boolean isClientInterview);

	Optional<List<Earmark>> findByAllocationAndProject(Employee employee, Project project);
	...
	Optional<List<Earmark>> findByAllocationAndOppurtunity(Long allocationId, String opportunityName);

	@Transactional
	@Modifying
	@Query("UPDATE Earmark p SET p.isActive = 'false' WHERE p.earmarkId in(:earmarkids)")
	Integer unEarmark(List<Long> earmarkids);

	@Transactional
	@Modifying
	@Query("UPDATE Earmark e SET e.isActive = 'false' WHERE e.earmarkId = :earmarkId")
	Integer unEarmark(Long earmarkId);

	Optional<List<Earmark>> findByAllocation(Allocation allocationId);

	Optional<List<Earmark>> findByAllocationAllocationIdAndIsActiveIsTrue(Long allocationId);
	...
	List<Earmark> findByProjectIsNotNullAndOpportunityIsNullAndIsActiveIsTrue();
	...
	List<Earmark> findByProjectIsNullAndOpportunityIsNotNullAndIsActiveIsTrue();
